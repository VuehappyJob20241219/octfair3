        ...
        <div class="backdrop">
            <div class="container">
                ...
                <div class="button-box">
                    <button v-on:click="qnaDetail.qnaIdx ? handlerUpdateBtn() : handlerSaveBtn()">{{qnaDetail.qnaIdx ? '수정' : '저장'}}</button>
...
const handlerGetModalDetail = () => {
    axios.post('/api/board/qnaDetailFileRe.do', {qnaSeq : props.idx})
        .then((res) => {
...
const handlerSaveBtn = () => {
    ...
    axios.post('/api/board/qnaFileSaveRe.do', formData)
        .then((res) => {
...
const handlerUpdateBtn = () => {
    ...
    if(fileData.value)
        formData.append('file', fileData.value);
    formData.append('text', new Blob([JSON.stringify(textData)], {type:'application/json'}));
    axios.post('/api/board/qnaFileUpdateRe.do', formData)
        .then((res) => {
            if(res.data.result.toUpperCase() === 'SUCCESS') {
                modalStore.setModalState();
                emits('postSuccess');
...
const handlerDeleteBtn = () => {
    axios.post('/api/board/qnaFileDeleteRe.do', {qnaSeq : props.idx})
        .then((res) => {
            modalStore.setModalState();
            emits('postSuccess');
        }
