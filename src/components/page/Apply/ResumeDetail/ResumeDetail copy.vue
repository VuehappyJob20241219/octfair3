...
const baseIdx = ref();
const isResumeNewPage = window.location.pathname.includes("resume-new.do");
const isResumeDetailPage = window.location.pathname.includes("resume-detail.do");
...
const {
  ...
  queryKey: ["resumeBaseInfo", userParam],
  queryFn: async () => {
    return await resumeBaseInfoApi(userParam);
  },
  enabled: !!isResumeNewPage,
});
